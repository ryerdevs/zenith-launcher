...
import { Wifi, WifiOff, Loader2, ShieldCheck, User, Play } from 'lucide-react'
...
export function LoginModal() {
  ...
  const avatarTarget = username.trim().length > 0 ? username : "MHF_Steve"
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      if (isOffline) {
        if (username.length < 3) throw new Error("Mínimo 3 caracteres.")
        
        // Intentar login (aunque sea offline, el backend puede validar cosas)
        try { await api.auth.loginOffline(username) } catch {}
        
        setTimeout(() => setUser(username, 'offline'), 500)
      } else {
  ...
  return (
    <div className="w-[340px] bg-card/95 backdrop-blur-sm rounded-2xl shadow-2xl border border-white/10 p-6 animate-in fade-in zoom-in-95 duration-500">
