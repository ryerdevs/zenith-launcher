...
export function EditInstanceDialog({ instanceId, isOpen, onClose, onSuccess }: EditInstanceDialogProps) {
    ...
    useEffect(() => {
        if (isOpen && instanceId) {
            ...
            if (target) {
                ...
                // Si target.image existe, lo usamos. Si no, usamos el default.
    ...
    const handleSave = async () => {
        ...
        setSaving(true)
        try {
            // Lógica para detectar si la imagen cambió
            const currentInstance = instances.find(i => i.id === instanceId)
            const originalImage = currentInstance?.image
            
            // Enviamos la imagen si es un archivo nuevo (base64) O si es un preset diferente al que tenía
        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-[650px] max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    ...
                    <DialogDescription>Modifica versiones, apariencia y opciones.</DialogDescription>
                </DialogHeader>
                ...
                <div className="grid gap-5 py-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="flex gap-4 h-48"> 
                            ...
                            >
                                {/* 
                                    IMPORTANTE: key={selectedImage} fuerza a React a recrear el elemento 
                                    cuando la imagen cambia, eliminando el "display: none" si hubo error antes. 
                                */}
                                <img 
                                    key={selectedImage} 
                                    src={selectedImage} 
                                ...
                                {/* Icono de fondo por si falla la carga */}
                                <ImageIcon className="text-zinc-800 w-12 h-12 absolute -z-10" />
                            </div>

                            {/* Grid de Presets con SCROLL PERSONALIZADO */}
