...
interface DataPacksManagerProps {
    instanceId: string
}

export function DataPacksManager({ instanceId }: DataPacksManagerProps) {
    ...
    const loadPacks = async () => {
        setLoading(true)
        try {
            const res = await fetch(`http://localhost:5000/api/instances/${instanceId}/datapacks`)
    ...
    return (
        <div className="flex flex-col h-full">
            <div className="flex justify-between items-center mb-4">
                <h3 className="text-sm font-medium text-muted-foreground">Data Packs ({packs.length})</h3>
                ...
                <ScrollArea className="h-full p-2">
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center h-full text-muted-foreground gap-2 p-8">
                            ...
                            <p className="text-sm">No hay data packs instalados</p>
                        </div>
                    ) : (
