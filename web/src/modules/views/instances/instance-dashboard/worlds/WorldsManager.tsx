...
export function WorldsManager({ instanceId }: WorldsManagerProps) {
    ...
    const filteredWorlds = worlds.filter(w => w.name.toLowerCase().includes(search.toLowerCase()))
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            <div className="flex items-center justify-between mb-4 gap-4">
                ...
                <Button variant="ghost" size="sm" onClick={loadWorlds} disabled={loading} className="text-muted-foreground">
            ...
            <div className="grid grid-cols-12 gap-4 px-4 py-2 text-xs font-medium text-muted-foreground border-b border-border/50 uppercase tracking-wider">
                <div className="col-span-8">Mundo</div>
                                ...
                                <div key={idx} className="grid grid-cols-12 gap-4 items-center px-4 py-3 hover:bg-muted/10 transition-colors group">
                                    <div className="col-span-8 flex items-center gap-3 overflow-hidden">
                                        ...
                                        <div className="flex flex-col min-w-0">
                                            <span className="font-semibold truncate text-foreground">{world.name}</span>
                                            <span className="text-xs text-muted-foreground">{formatSize(world.size)}</span>
                                        </div>
                                    </div>
